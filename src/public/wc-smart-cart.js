	...
	let wcSmartCartInit = {
		...
		showSmartCart: function () {
			jQuery( document.body ).on(
				'added_to_cart wc-blocks_added_to_cart',
					...
					jQuery.ajax( {
						...
						success: function ( data ) {
							if ( data.success ) {
								...
								jQuery(
									'#wc-smart-cart-container .wc-smart-cart'
								).animate( { right: '30px' }, 'slow' );

								// Attach click event to close button
								jQuery(
									'#wc-smart-cart-container .wc-smart-cart-close'
								).on( 'click', function ( e ) {
									e.preventDefault();
		...
		closeSmartCart: function () {
			jQuery( '#wc-smart-cart-container .wc-smart-cart' ).animate(
				{ right: '-330px' },
				'slow',
				function () {
